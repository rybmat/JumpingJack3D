...
void main(void) {
	...
	vec4 La = vec4(1,1,1,1);        //światło otoczenia
	vec4 Ma = vec4(texColor0.rgb*0.2,1);//vec4(0,0,0,1);        //materiał dla światła otoczenia
	...
	vec4 Ms = (9*texColor1 + vec4(1,1,1,1))/10;        //materiał dla światła odbijanego
